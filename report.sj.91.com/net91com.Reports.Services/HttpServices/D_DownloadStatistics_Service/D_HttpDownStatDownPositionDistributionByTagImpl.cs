...
{
    public class D_HttpDownStatDownPositionDistributionByTagImpl : Abs_HttpDownLoad<D_StatDownPositionDistribution>
    {
        ...
        protected override List<List<T>> GetData<T>(HttpContext context, bool isfortable = false)
        {
            ...
            var lists = new List<List<D_StatDownPositionDistribution>>();

            var result = D_StatDownPositionDistribution_Service.Instance.GetD_StatDownPositionDistributionByTagCache(restype, softs,
                                                                                                      platform,
                                                                                                      projectsource,
                                                                                                      version,
                                                                                                      downtype, 
        ...
        protected override List<List<string>> ObjectParseListFillDetailTable<T>(List<T> list,bool DetailEqChart=true)
        {
            ...
            if (list is List<D_StatDownPositionDistribution>)
            {
                ...
                foreach (D_StatDownPositionDistribution item in temp)
                {
                    ...
                    //pAndP = item.PositionName.Equals("0") ? "未知" : pAndP;
                    values.Add(pAndP);
        ...
        {
            get { return "D_HttpDownStatDownPositionDistributionByTagImpl"; }
        }

        public override string ServiceName
        {
            get { return "下载位置查看按专辑"; }
        }
