...
{
    public class D_HttpDownStatDownPositionDistributionDetailImpl : Abs_HttpDownLoad<D_StatDownPositionDistribution>
    {
        ...
        public override Result GetChart<T>(HttpContext context)
        {
            ...
            lines.Add(new LineChartLine { Name = "下载位置分布明细", Points = points, Show = true });
        ...
        protected override List<List<T>> GetData<T>(HttpContext context, bool isfortable = false)
        {
            ...
            int isTagClassDetail = Convert.ToInt32(context.Request["isTagClassDetail"]);
            string pagename = context.Server.UrlDecode(context.Request["pagename"] ?? "");
            ...
            if (0==isTagClassDetail)
            {
                result = D_StatDownPositionDistribution_Service.Instance.GetD_StatDownPositionDistributionDetailCache(restype, softs,
                                                                                                         platform,
                                                                                                         projectsource,
                                                                                                         version,
                                                                                                         downtype,
                                                                                                         begintime,
                                                                                                         endtime,
                                                                                                         period, positionid, pagename, areaid, isdiffpagetype, stattype);
                if (result.Count != 0)
                    lists.Add(result);
            }
            else
            {
                result = D_StatDownPositionDistribution_Service.Instance.GetD_StatDownPositionDistributionByTagClassCacheDetail(restype, softs,
                                                                                         platform,
                                                                                         projectsource,
                                                                                         version,
                                                                                         downtype,
                                                                                         begintime,
                                                                                         endtime,
                                                                                         period, pagename,areaid,isdiffpagetype,stattype);
                if (result.Count != 0)
                    lists.Add(result);
            }
        ...
        {
            get { return "D_HttpDownStatDownPositionDistributionDetailImpl"; }
        }

        public override string ServiceName
        {
            get { return "新下载位置分布按专辑"; }
        }
