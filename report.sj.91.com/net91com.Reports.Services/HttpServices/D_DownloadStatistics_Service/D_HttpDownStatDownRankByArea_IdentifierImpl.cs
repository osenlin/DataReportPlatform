...
{
    public class D_HttpDownStatDownRankByArea_IdentifierImpl  : Abs_HttpDownLoad<D_StatDownRank_SUM>
    {
        ...
        protected override List<List<T>> GetData<T>(HttpContext context, bool flag = false)
        {
            ...
            var lists = new List<List<D_StatDownRank_SUM>>();

            List<D_StatDownRank_SUM> resultbefore = D_StatDownRank_Service.Instance.GetD_StatDownRankByAreaByCache(
                                                                                            restype, 
                                                                                            softs, 
                                                                                            platform,
                                                                                            comparebeforetime, 
            ...
            List<D_StatDownRank_SUM> resultrestime = D_StatDownRank_Service.Instance.GetD_StatDownRankByAreaByCache(restype, softs, platform,
                                                                                            restime, period, areaid, downtype);

            var resulttemp = (from  itrestime in resultrestime
                              join itbefore in resultbefore on itrestime.ResIdentifier equals itbefore.ResIdentifier
                              into os
            ...
            var result = (from item in resulttemp
                          join itemresinfo in lstresinfo 
                          on item.ResIdentifier equals itemresinfo.ResIdentifier into os
                          from t in os.DefaultIfEmpty()
                          select new D_StatDownRank_SUM
                          {
        ...
        {
            get { return "D_HttpDownStatDownRankByArea_IdentifierImpl"; }
        }
        ...
        {
            get { return "Reports_New/D_DownlaodStatistics/D_ResDownLoadRankByArea_Identifier.aspx"; }
        }
