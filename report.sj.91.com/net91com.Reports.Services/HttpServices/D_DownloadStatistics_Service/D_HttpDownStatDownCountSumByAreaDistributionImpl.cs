...
{
    class D_HttpDownStatDownCountSumByAreaDistributionImpl : Abs_HttpDownLoad<D_StatDownCountsBySoft_SUM>
    {
        public override string ServiceCategory
        {
            get { return "D_HttpDownStatDownCountSumByAreaDistributionImpl"; }
        }
        ...
        {
            get { return "reports_new/d_downlaodstatistics/D_ResDownLoadSumByAreaDistributionl.aspx"; }
        }
        ...
        protected override string[] DownloadExcelDataTableTitleName
        {
            get
            {
                if (softid == 46 || 85 == softid || 46 == softid)
                {
                    return new string[]
                         {
                             "排名","地区名称", "所有", "下载点击(去除更新)", "下载点击（来自更新）", "下载点击(来自搜索)",
                ...
                else
                {
                    return new string[]
                         {
                             "排名","地区名称", "下载点击（所有）", "下载点击(去除更新)", "下载点击(静默更新)",
        ...
        public override Result GetChart<T>(HttpContext context)
        {
            ...
            int showtype = int.Parse(context.Request["showtype"]);
            ...
            if (lists.Count != 0)
            {
                ...
                {
                    sb.AppendFormat("['{0}',{1}],", item.CountryName, GetValueType(item,showtype));
                }
        ...
        protected override List<List<T>> GetData<T>(HttpContext context, bool flag = false)
        {
            ...
            {
                var result = D_StatDownCountSum_Service.Instance.GetD_StatDownByAreaDistribution_SUMByCache(restype, softs,
                                                                                                platform, begintime,
                                                                                                endtime,
