...
{
    public class Sjqd_ChannelAdmins
    {
        #region 构造函数
        ...
        /// 来自数据库的数据构造
        /// </summary>
        public Sjqd_ChannelAdmins(IDataReader reader)
            : this()
        {
            LoadFromDb(reader);
        }
            ...
            for (int i = 0; i < reader.FieldCount; i++)
            {
                switch (reader.GetName(i).ToLower())
                {
                    case "id":
                        ...
                        break;
                    case "username":
                        UserName = reader.IsDBNull(i) ? String.Empty : reader.GetString(i);
                        break;
                    case "softid":
