<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="D_ResDownloadAvgCountByVersion.aspx.cs" Inherits="net91com.Stat.Web.Reports_New.D_DownlaodStatistics.D_ResDownloadAvgCountByVersion" %>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        var modeType=createSelectControl("selectmodetype", '请选择', false, function () {
    ...
    function getChart() {
        ...
        $.ajax({
            ...
            dataType: "json",
            data: {
                'service': 'D_HttpDownStatDownAvgCountByVersionImpl',
                'do': 'get_chart', 
                'endtime': $("#endTime").val(),
    ...
    function getTabsControl() {
        ...
        var options= $("#selectversion").find("option:selected");
        for (var i = 0; i < options.length; i++) {
            var obj = new Object();
            obj.id = options[i].value;
            obj.value = options[i].text;
            arr.push(obj); 
        }   
        ...
        var multitype=$("#hiddenmultitype").val();
     
        if (multitype==2) {
            for (var i = 0; i < arr.length; i++) {
                html += " <li><a onclick=\"gettabs('" + arr[i].id + "','"+ arr[i].value +"')\"><font>" + arr[i].value + "</font></a></li>";
            }
        } else {
        ...
        if (arr.length == 0) { }
        else if (arr.length > 1 || arrchannel.length>1) {
            $("#mytags2").html(html);
            mytabs2 = createTabs($("#mytags2"), 0);
            mytabs2.click(0);
        }
        else {
            gettabs(arr[0].id,arr[0].value);
            $("#mytags2").html("");
        }
    }
    ...
    function gettabs(value,name) {
        ...
        oTable = $('#mytable').dataTable({
            ...
            "fnServerParams": function (aoData) {
                aoData.push({ "name": "service", "value": "D_HttpDownStatDownAvgCountByVersionImpl" });
                ...
                aoData.push({ "name": "modetype", "value": 2 });
                aoData.push({ "name": "showtype", "value": 3 });
                ...
                aoData.push({ "name": "mutitype", "value": $('#hiddenmultitype').val() });
            },
    ...
    function setDownExcel() {
        var url = serverUrl + "?do=get_excel&stattype=1&service=D_HttpDownStatDownAvgCountByVersionImpl&projectsource=-1&softs=" + $("#selectsofts").val()
            + "&plat=" + $("#selectplats").val() + "&begintime=" + $("#beginTime").val() + "&endtime=" + $("#endTime").val()
            + "&restype=" + $("#selectrestype").val() + "&version="+$("#selectversion").val() +"&modetype=" + $("#selectmodetype").val() + "&showtype=" + 3
            + "&channelids=" + channelControl.getSelectChannels() + "&channelnames=" + $("#mychanneltxt").val()+"&areaid="+$("#selectArea").val() +"&areatype="+ g_areatype  
            + "&singlehiddenvalue=" + $("#hiddenvalue").val() + "&singlehiddenname=" + $("#hiddenname").val()+"&period="+$("#hiddenperiod").val()+"&mutitype="+ $('#hiddenmultitype').val();
        ...
        <div style="padding: 10px;">
            <table id="table1" cellpadding="0" cellspacing="10" border="0" width="100%">
                <tr>
                    ...
                    <td style="width: 200px;">
                        <select id="selectversion" style="width: 150px; display: none" multiple="multiple">
                        </select>
                    </td>
                    <td></td>
                        ...
                        <div class="selectdisplay" style="width: 100%">
                            <div id="divareatxt">
                                    <select id="selectArea" style="width: 150px;"  >
                                         <option selected="selected" value="-1">不区分地区</option>
                                         <option  value="9">埃及</option>
                                         <option  value="25">巴基斯坦</option>
                                         <option  value="58">俄罗斯</option>
                                         <option  value="69">菲律宾</option>
                                         <option  value="137">马来西亚</option>
                                         <option  value="184">沙特阿拉伯</option>
                                         <option  value="202">泰国</option>
                                         <option  value="235">伊拉克</option>
                                         <option  value="239">印度</option>
                                        <option  value="245">越南</option>
                                    </select>
                            </div>
                        </div>
                    </td>
...
<script type="text/javascript">
    ...
    //    var selectHtml = "<option selected='selected' value='-1'>不区分地区</option>";
