<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="D_ResDownLoadSumBySoft.aspx.cs" Inherits="net91com.Stat.Web.Reports_New.D_DownlaodStatistics.D_ResDownLoadSumBySoft" %>
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head runat="server">
        ...
        <script type="text/javascript">
            ...
            function areaMulti(event, ui) {
                //当为取消选项
                if (ui.checked==false || getContronlValueLength("selectArea")<1) {
                    return true;
                }
                ...
                if (getContronlValueLength("selectshowtype") > 1) {
                    alert('不允许多个维度同时多选，展示类型已经多选');
                    return false;
                }
                $("#hiddenmultitype").val(4);
                //设置渠道只能单选
            ...
            function showtypeMulti(event, ui) {
                ...
                if (getContronlValueLength("selectArea") > 1) {
                    alert('不允许多个维度同时多选,地区已经多选');
                    return false;
                }
            ...
            function versionMulti(event, ui) {
                ...
                if (getContronlValueLength("selectshowtype") > 1 || getContronlValueLength("selectArea") > 1) {
        ...
        <script type="text/javascript">
            ...
            var AreaJson = <%= AreaJsonForEnShortName %>;
            ...
            var tableHtml_46 = "<tr><th rowspan=\"2\" style=\"width: 120px\">日期</th> <th colspan=\"5\" >下载点击</th> <th colspan=\"3\" >下载成功</th> <th colspan=\"3\" >下载失败</th> <th colspan=\"3\">安装成功</th> <th colspan=\"3\">安装失败</th><th colspan=\"3\">游戏下载</th></tr><tr><th>所有</th><th>去除更新</th>";
            tableHtml_46 += "<th>来自更新</th><th>来自搜索</th><th >静默更新</th><th >所有</th><th >更新</th><th >静默更新</th><th >所有</th><th >更新</th><th >静默更新</th>";
            tableHtml_46 += "<th >所有</th><th >更新</th><th >静默更新</th><th >所有</th><th >更新</th><th >静默更新</th><th >所有</th><th >一次分发</th><th >二次分发</th></tr>";
            ...
            $(function() {
                ...
                AreaControl = createSimpleSelectControl("selectArea", '请选择区域', true, function() {
                    ...
                    if ((getContronlValueLength("selectversion") == 1 || getContronlValueLength("selectversion") == 0)
                        && getContronlValueLength("selectshowtype") == 1 && getContronlValueLength("selectArea") == 1) {
            ...
            function getSetting(softid) {
                var defaultSetting={
                    ...
                    "fnServerParams": function(aoData) {
                        aoData.push({ "name": "service", "value": "D_HttpResDownLoadSumBySoftImpl" });
                        ...
                        aoData.push({ "name": "areaid", "value": getareas() });
                        aoData.push({ "name": "areatype", "value": g_areatype });
                        aoData.push({ "name": "multype", "value": $("#hiddenmultype").val() });
                    },
                ...
                if (softid == 46 || softid==85 || softid==58) {
                    defaultSetting.aoColumns = [
                        ...
                        { "sClass": "tdright","aTargets" : [ 19 ] },
                        { "sClass": "tdright","aTargets" : [ 20 ] }
                    ];
            ...
            function getChart() {
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    data: {
                        'service': 'D_HttpResDownLoadSumBySoftImpl',
                        'do': 'get_chart',
                        'endtime': $("#endTime").val(),
            ...
            function getDetailTableTitle() {
                ...
                } else if ($("#selectmodetype").val() == "2") {
                    arr = getConditionArray("selectversion");
                } else if ($("#selectmodetype").val() == "3") {
                    arr = getConditionArray("selectprojectsources");
                }
                ...
                var detailTable;
                var arr_area = getConditionArray("selectArea");
                var arr_showtype = getConditionArray("selectshowtype"); //new Array();    
                ...
                if (2 == multype) {
                    ...
                    html += " <li><a onclick=\"gettabs('" + arr_showtype[0].id + "','" + arr_showtype[0].value + "',3)\"><font>" + arr_area[0].value + "</font></a></li>";
                } else if (4 == multype || 1 == multype) {
                    for (var i = 0; i < arr_area.length; i++) {
                        html += " <li><a onclick=\"gettabs('" + arr_area[i].id + "','" + arr_area[i].value + "',4)\"><font>" + arr_area[i].value + "</font></a></li>";
                    }
                }
                $("#mytags2").html(html);
                detailTable = createTabs($("#mytags2"), 0);
                detailTable.click(0);
                if (arr.length>1||arr_area.length>1) {
                    $("#mytags2").show();
                } else {
                    $("#mytags2").hide();
                }
            ...
            function setDownExcel() {
                ...
                var url = serverUrl + "?do=get_excel&service=D_HttpResDownLoadSumBySoftImpl&projectsource=" + $("#selectprojectsources").val() + "&softs=" + $("#selectsofts").val()
                    ...
                    + "&singlehiddenvalue=" + $("#hiddenvalue").val() + "&singlehiddenname=" + $("#hiddenname").val() + "&period=" + $("#hiddenperiod").val() + "&areaid=" + getareas() + "&areatype=" + g_areatype + "&multype=" + $("#hiddenmultype").val();
    ...
    <body>
        ...
        <form id="form1" runat="server">
            <div class="maindiv">
                <!--只用于前台的标识-->
                <input type="hidden" value="1" id="hiddenmultitype" />
                ...
                <div style="padding: 10px;">
                    <table id="table1" cellpadding="0" cellspacing="10" border="0" width="100%">
                        <tr>
                            ...
                            <td style="width: 200px;">
                                <div id="divversion" class="modetypeclass">
                                    <select id="selectversion" style="width: 150px; display: block" multiple="multiple">
                                        <option value="-1" selected="selected">不区分版本</option>
                                    </select>
                                </div>

                                <div id="divprojectsources" class="modetypeclass">
                                    <select id="selectprojectsources" style="width: 150px; display: none">
                                        <option value="-1" selected="selected">不区分来源</option>
                                    </select>
                                </div>

                                <div id="divchanneltxt" class="modetypeclass">
                                    <input type="text" value="" id="mychanneltxt" class="txtbox " style="height: 20px;" />
                                </div>
                            </td>
                            <td></td>
                        </tr>
                        <tr>
                            <td>从：
                                <input  style="width: 78%" class="ui-multiselect ui-widget ui-state-default ui-corner-all" type="text" id="beginTime" class="Wdate" value="<%= BeginTime.ToString("yyyy-MM-dd") %>" onclick=" WdatePicker() " />
                          
                            </td>  <td>   
                                 到：
                                <input style="width: 78%" class="ui-multiselect ui-widget ui-state-default ui-corner-all" type="text" id="endTime" class="Wdate" value="<%= EndTime.ToString("yyyy-MM-dd") %>" onclick=" WdatePicker() " />
                            </td>
                            <td>
                            ...
                            <td colspan="2" style="width: 120px">
                                <div style="position: relative; z-index: 100">
                                    <select id="selectArea" style="width: 150px;" multiple="multiple">
                                    </select>
                                </div>
                            </td>
                            <td>
