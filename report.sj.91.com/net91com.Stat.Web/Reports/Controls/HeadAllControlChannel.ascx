<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="HeadAllControlChannel.ascx.cs"
    Inherits="net91com.Stat.Web.Reports.Controls.HeadAllControlChannel" %>
<%@ Register Src="ChanelForAll.ascx" TagName="Channel" TagPrefix="uc2" %>
...
<script type="text/javascript">
    ...
    function drawversionsdoms(softid, platid,first) {
        ...
        $.ajax({
            type: "get",
            url: <%= NewVersionInfo 
                      ? "\"/Reports_New/HttpService.ashx?service=UtilityService&do=getversionbysoftidnew&softs=\"+softid+\"&platform=\"+platid"
                      : "\"/Reports/HanderForOption.ashx?action=getversionforsession&soft=\"+softid+\"&platform=\"+platid+\"&versiontype=" + VersionType + "\"" %>,
            success: function (data) {
                var mydata = eval("(" + data + ")");
                 ...
                 <%} %> 
                 <% if (NewVersionInfo) { %>
                    canviewversion = mydata.data;
                    for (var  i = 0; i < canviewversion.length; i++) {
                    myselectversion.options.add(new Option(canviewversion[i].Value, canviewversion[i].ID));
                }
                 <%} else { %>
                ...
                if (!selected && myselectversion.options.length > 0) {
                    myselectversion.options[0].selected = true;
                }
                ///第一次不需要这个
                if (versionbox) {
        ...
        softbox = $("#myselectsoft").multiselect({
            ...
            beforeclose: function () {
                ...
                if (ischangedsoft != $(".inputsoftselect").val()) {
                    ...
                    softids_Channel= $(".inputsoftselect").val();
                    clear();
                    hasloadData_Channel=true;
                }
                <%} %>
        ...
        platbox = $("#myselectplat").multiselect({
            ...
            beforeclose: function () {
                ...
                if (ischangedplat != $(".inputplatformselect").val()) {
                     ///设置渠道参数
                    platforms_Channel= $(".inputplatformselect").val();
                    ///设置平台他的平台传入底层
                    $("#<%=Channel1.ClientID %>_hidplatform").val(platforms_Channel);
                    //clear();
                    //hasloadData_Channel=true;
                   <%if(!HiddenVersion){ %>
                    ///获取版本
...
<div class="head">
    ...
    <%} %>
    <%} %>
    <uc2:Channel ID="Channels" runat="server" />
    <%if (ShowCheckBox)
      { %>
